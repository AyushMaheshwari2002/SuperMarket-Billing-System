...
void genrate_receipt()
{
    ...
    system("pause > nul");                 // if we use this then "Press any key to continue" line shows up on the screen
    main();
}
...
bool is_existed(string item)
{
    ...
    ifstream obj("file.txt");           // ifstream provides input operation
    string c;
    bool found;
    while(getline(obj,c,','))
    {
...
void add_element()
{
    ...
    fstream obj("file.txt",ios::out|ios::app);          //ios::app set the stream position indicator to the end of the stream before each output operation
    cout<<"\n\t\tEnter Item Details\n\t..................................\n\n\n";
    cout<<"\t\tEnter Item-ID. : ";
    cin>>item_no;
...
void delete_item(string item)
{
   ...
   if(is_existed(item)==1)
   {
        ...
        int n=0;
        ofstream obj1;
        obj1.open("file.txt",ios::trunc);          //"trunc" replace the value of its first argument with another value that has a smaller or same precision
        ifstream tmp;
        tmp.open("tmp.txt",ios::in);        //"in" allow you to easily test if an expression matches any value in a list of values
